...
#define tState      data[0]
#define tMonId      data[1]
#define tOldClass  data[2]
#define tNewClass  data[3]
#define tOldFunc    4

void Task_ClassRing(u8 taskId)
{
    static const u8 askText[] = _("It might affect {STR_VAR_1}'s class.\nAre you sure you want to use it?");
    static const u8 doneText[] = _("{STR_VAR_1}'s class have changed due\nto the effects of the {STR_VAR_2}!{PAUSE_UNTIL_PRESS}");
    s16 *data = gTasks[taskId].data;
    ...
    switch (tState)
    {
        ...
        case 4:
            ...
            break;
        case 5:
            SetMonData(&gPlayerParty[tMonId], MON_DATA_CLASS, &tNewClass);
            // CalculateMonStats(&gPlayerParty[tMonId]);
            RemoveBagItem(gSpecialVar_ItemId, 1);
            gTasks[taskId].func = Task_ClosePartyMenu;
            break;
...
void ItemUseCB_ClassRing(u8 taskId, TaskFunc task)
{
    s16 *data = gTasks[taskId].data;

    tState = 0;
    tMonId = gPartyMenu.slotId;
    tOldClass = GetMonData(&gPlayerParty[tMonId], MON_DATA_CLASS);
    tNewClass = GetItemSecondaryId(gSpecialVar_ItemId);
    SetWordTaskArg(taskId, tOldFunc, (uintptr_t)(gTasks[taskId].func));
    gTasks[taskId].func = Task_ClassRing;
}
