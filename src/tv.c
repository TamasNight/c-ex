...
u8 CheckForPlayersHouseNews(void)
{
    ...
    if (gSaveBlock1Ptr->location.mapGroup != MAP_GROUP(MAP_PASCOLI_DEL_LAGO_BRENDANS_HOUSE_1F))
        return PLAYERS_HOUSE_TV_NONE;

    // Check if not in player's house (dependent on gender)
    if (gSaveBlock2Ptr->playerGender == MALE)
    {
        if (gSaveBlock1Ptr->location.mapNum != MAP_NUM(MAP_PASCOLI_DEL_LAGO_BRENDANS_HOUSE_1F))
            return PLAYERS_HOUSE_TV_NONE;
    }
    else
    {
        if (gSaveBlock1Ptr->location.mapNum != MAP_NUM(MAP_PASCOLI_DEL_LAGO_MAYS_HOUSE_1F))
            return PLAYERS_HOUSE_TV_NONE;
    }
